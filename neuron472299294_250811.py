...
class Neuron472299294:
    ...
    def update_factors(self, f_dict, all_rev=False):
        n_dict = {}
        if all_rev:
...
#    def _set_mechanism_parameters(self, e_pas=-90.582359314, ena=53.0, ek=-107.0):
    def _set_mechanism_parameters(self, e_pas=-70, ena=30, ek=-107.0):       
        ...
        for sec in self.apic:
            ...
            sec.cm = 2.17*0.38*self.f_cm
            #sec.g_pas = 7.76313565885e-05*self.f_g_pas
            sec.g_pas = 7.76313565885e-05*0.5*self.f_g_pas
        for sec in self.axon:
            #sec.cm = 1.0*self.f_cm
            sec.cm = 1.0*0.38*self.f_cm
            #sec.g_pas = 0.000551976131423*self.f_g_pas
            sec.g_pas = 0.000551976131423*0.5*self.f_g_pas
        for sec in self.dend:
            sec.cm = 2.17*0.38*self.f_cm
            #sec.cm = 2.17*self.f_cm
            #sec.g_pas = 1.08526630687e-05*self.f_g_pas
            sec.g_pas = 1.08526630687e-05*0.5*self.f_g_pas
        for sec in self.soma:
            ...
            sec.gbar_NaTs = 0.452286*0.7*self.f_gbar_NaTs
            #sec.gbar_Nap = 0.00101181*self.f_gbar_Nap
            sec.gbar_Nap = 0.00101181*0.2*self.f_gbar_Nap
            ...
            sec.g_pas = 0.000199625*0.5*self.f_g_pas
